...
# display names expected by Flask App Builder AUTH_ROLES_MAPPING.
...
def _build_roles_mapping(idp_group_config: dict[str, str]) -> dict[str, list[str]]:
    ...
    for role_key, raw_groups in idp_group_config.items():
        ...
        for group in [group.strip() for group in raw_groups.split(",") if group.strip()]:
...
class WebserverConfigGenerator:
    ...
    Produces a Jinja2 template string (non-sensitive values baked in,
    client_secret as a placeholder) and a separate dict of sensitive values
    to be distributed as a relation secret.
    """
    ...
    def _provider_info(self) -> OauthProviderConfig | None:
        ...
        # self._charm._oauth_active ensures successful retrieval of provider info
    ...
    def webserver_config_template(self) -> str | None:
        ...
        if missing:
            ...
            raise WebserverConfigError("Missing OauthProviderConfig fields")
        ...
        auth_roles_mapping = _build_roles_mapping(
            {
                "admin": str(self._charm.config.get(constants.IDP_GROUPS_FOR_ADMIN_CONFIG, "")),
                "op": str(self._charm.config.get(constants.IDP_GROUPS_FOR_OP_CONFIG, "")),
                "user": str(self._charm.config.get(constants.IDP_GROUPS_FOR_USER_CONFIG, "")),
                "viewer": str(self._charm.config.get(constants.IDP_GROUPS_FOR_VIEWER_CONFIG, "")),
                "public": str(self._charm.config.get(constants.IDP_GROUPS_FOR_PUBLIC_CONFIG, "")),
            }
        )
        ...
        try:
            ...
            return template.render(
                ...
                enable_user_registration=self._charm.config[
                    constants.ENABLE_USER_REGISTRATION_CONFIG
                ],
            )
        except TemplateError as e:
            logger.exception("Failed to render webserver_config template: %s", e)
            raise WebserverConfigError("Failed to render webserver_config_template")
