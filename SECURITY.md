# Security Policy — Charmed Airflow Coordinator
...
Charmed Airflow Coordinator follows Canonical's standard charm support lifecycle, aligned with the Ubuntu LTS base. The product currently ships interim releases; no LTS commitment is made at this time.
...
Vulnerabilities are tracked via [GitHub Security Advisories](https://github.com/canonical/airflow-coordinator-k8s-operator/security/advisories) and coordinated internally with Canonical's Product Security Incident Response Team (PSIRT).

Note that this charm coordinates other Charmed Airflow components and packages the upstream [Apache Airflow](https://github.com/apache/airflow). Vulnerabilities in the upstream Apache Airflow project itself should be reported to the [Apache Security Team](https://www.apache.org/security/) directly; Canonical will track and apply upstream fixes to supported tracks.
